...
| 2.2 | **Metadata persistence — RESOLVED** | M6 needs to list "uploaded files" and show video metadata (codec, duration). S3/MinIO object listing alone is slow and carries no application-level metadata. | Use **SQLite** (file-based, fits cleanly into the `backend` container/volume, no extra service in docker-compose). `uploads` table row is created **at upload-start (M1)**, not just on completion — columns: id (= tus upload ID), filename, size, mime type, status (`uploading`/`paused`/`success`/`error`/`abandoned`), storage key, `last_seen` (for §2.11 cleanup), `bytes_received` (for M4's SSE snapshot, §8.12), and codec/resolution/duration (populated post-upload via `ffprobe`, M6). |
| 2.3 | **Test file strategy — RESOLVED** | A single approach can't cover both "large arbitrary-content files" (M1, M2, M3, M4, M5) and "small files with real, probeable video codecs" (M6). | **M1–M5**: synthetic files generated **on the fly** at test-run time (`fallocate`/`dd`/Node script) for 100MB/200MB/1000MB/2000MB/2100MB — content doesn't matter, only size, since these tests validate transfer/resume/throughput mechanics, not codec handling. Generated into a tmp location, deleted after the run. **M6 onward**: requires a **different approach** — small (few-second) *real* video files with actual codec streams that `ffprobe` can inspect, since random bytes aren't valid video. These are small enough (a few MB) to be **committed as fixtures** in `tests/fixtures/` rather than generated. |
...
| 2.5 | **Batch error behavior (M5)** — **RESOLVED** | M5 processes the batch **sequentially, one file at a time**. If a file errors, the batch pauses and offers retry/skip for that file before continuing to the next. | Implement queue as a sequential pipeline (not parallel). Parallel/independent-file processing is explicitly deferred to a future milestone — design the queue manager so it *could* be extended to parallel later (e.g., keep per-file state isolated), but don't build concurrency now. |
| 2.6 | **Upload concurrency (M5)** — **RESOLVED** | Sequential, one file at a time (per 2.5). | No concurrency cap needed for M5. Note for future milestone: introducing parallelism later. |
| 2.7 | **"Codecs mismatch and video file identification" (M6)** — **RESOLVED** | Detect-and-fallback only, **no transcoding**. | Use `ffprobe` to detect codec on upload-complete, compare against a browser-compatible allowlist (e.g., H.264+AAC in MP4, VP8/VP9/AV1 in WebM), and show a "format not supported for preview" placeholder if it doesn't match. Transcoding pipeline is out of scope entirely (not just deferred). |
| 2.8 | **Streaming/seeking support (M6)** | "Retrieve API so uploaded files can be streamed back" — for `<video>` seeking to work, the retrieve endpoint **must** support HTTP `Range` requests (206 Partial Content), proxying ranged `GetObject` calls to MinIO. Not explicitly stated but functionally required. | Add explicitly as a requirement in M6. |
| 2.9 | **File type scope (M6) — RESOLVED** | Platform is **video-only**. | All uploads must be video files, enforced via an **allowlist** of accepted file types — **MKV and MP4 for now** (extensible later). Enforced at upload time in M1 (see M1 Backend/Frontend), not deferred to M6. M6's "uploaded files" list and player can assume every entry is a video. |
...
│   ├── fixtures/              # Small real video files for M6 (§2.3) — committed, few MB total
...
### Frontend
- Status model per file: `idle | uploading | paused | error | success`, each with a distinct visual indicator (icon + color, no aesthetic polish needed). (M4 — §8.13 — later amends this to surface `abandoned` as a user-visible state too, via the SSE channel.)
...
**Scope:** no new application features. This milestone adds the Toxiproxy-based test infrastructure and a battery of scenario tests that exercise M1 (single upload) and M2 (resume/pause/heartbeat) under degraded **browser↔backend** network conditions — particularly speed/throughput, since that's the connection a real end-user's network actually affects. Subsequent milestones (M4 onward) continue to build on M2's feature set unaffected (§2.15).
...
## 8. Milestone 4 — Backend-Pushed Live Progress (SSE) (New)

**Scope:** everything in M2 (M3 — §7 — is a testing-only milestone and adds no application features, so feature dependencies build on M2 — same convention used for M5/Batch), plus a **server → client push channel** that reports *server-confirmed* upload progress and status. This is purely additive — it does **not** change the tus upload transport, the resume logic, or anything built in M1–M3. It exists to satisfy the requirement that progress be "updated automatically from the backend, with no manual refresh," and to make the **local-progress vs. server-confirmed-state** distinction explicit. M5 (Batch) builds its per-file and aggregate progress UI on this channel from the start, rather than wiring it client-side and reworking it later.
...
| 8.11 | **SSE during dropped-upload + resume, and connection reset** | When the upload is interrupted and resumed (M2 / §2.4 abort, or M3's `reset_peer` toxic), the feed must not report a false `success`/`error`; and a reset that also kills the SSE stream must recover. | The server-confirmed offset should **stall** during the interruption then **resume advancing** on reconnect — no spurious terminal event. If a `reset_peer` toxic resets the shared browser↔backend connection (killing both the upload and the SSE stream), assert the SSE stream auto-reconnects and re-syncs via the snapshot (8.6) alongside M2's resume. |
| 8.12 | **Schema gap: `bytes_received` column — NEW** | The snapshot-on-connect (8.6) needs to read "confirmed bytes received" per upload from the `uploads` table — but tus tracks the offset internally (via `@tus/server`/`@tus/s3-store`), not in our SQLite table. No such column exists yet (§2.2 didn't include one). | **Migration**: add a `bytes_received` column to `uploads` (§2.2, updated here). Updated on every `POST_RECEIVE` (same throttling as 8.4, so the column and the SSE emission stay in lockstep) and on `POST_FINISH` (set to `bytes_total`). The snapshot-on-connect (8.6) reads this column directly — no separate tracking mechanism needed. |
| 8.13 | **Status model amendment: `abandoned` becomes user-visible — NEW** | M2's frontend status model (`idle \| uploading \| paused \| error \| success`) never surfaced `abandoned` to the UI — it was a backend-only cleanup status for sessions the user had already left. The SSE channel now pushes `abandoned` events to **any** connected client (e.g., a second tab watching the same upload). | **Amend M2's status model** to add `abandoned` as a fifth user-visible state (small addition — one more icon/color). In the common case (the user who abandoned the upload has left), no one sees it; but if another tab/session is watching the same `uploadId`, it now correctly reflects that the session was cleaned up server-side rather than silently going stale. |

### Backend (Node/Express + tus)

- New endpoint **`GET /progress/stream`** with `Content-Type: text/event-stream`:
  - On connect, register the response in an in-memory set of subscribers, and **immediately send a snapshot** of every non-terminal `uploads` row (`uploadId`, `bytesReceived` = `bytes_received` column per 8.12, `bytesTotal` = size, `status`).
...
- **`uploads` table migration** (8.12): add `bytes_received` column (default 0), updated on `POST_RECEIVE` (throttled per 8.4) and set to `bytes_total` on `POST_FINISH`.
- **In-process event emitter** wired to the tus server:
  - On `POST_RECEIVE` → throttled (per 8.4) `progress` event: `{ uploadId, bytesReceived, bytesTotal, status: 'uploading' }`, and the `bytes_received` column update (8.12).
  - On `POST_FINISH` → terminal event `{ uploadId, status: 'success', bytesReceived = bytesTotal }`.
  - On the cleanup/abandon path (from M2) → `{ uploadId, status: 'abandoned' }` (now user-visible per 8.13).
  - On error → `{ uploadId, status: 'error', message }`.
- Each SSE message uses an incrementing `id:` field so `Last-Event-ID` works on reconnect (the snapshot-on-connect in 8.6 is the primary resync mechanism; the id is a backstop).
...
- The per-file progress bar may display the optimistic local value from tus `onProgress`, but the **status badge** — now `idle | uploading | paused | error | success | abandoned` (per 8.13's amendment to M2's model) — is driven by the SSE map, not inferred locally.
...
- **Progress emission (integration):** drive a tus upload; assert the SSE client receives `progress` events for the right `uploadId` with monotonically increasing `bytesReceived`, that emission rate is bounded (throttling per 8.4), and that the `uploads.bytes_received` column (8.12) tracks the same value.
- **Terminal events:** assert `success`, `error`, and `abandoned` events are each emitted on the corresponding backend path; assert `abandoned` is now visible in the frontend's status badge (8.13).
...
- **SSE channel resilience (8.9):** drop the SSE request mid-upload (close the `EventSource`, or use the §2.4 abort harness against that request); assert the *upload* request — which is independent — is unaffected, and the SSE stream auto-reconnects and re-syncs via the snapshot.
- **Dropped-upload visibility (8.11):** during an M2 dropped-upload + resume (§2.4 harness on the upload request), assert the SSE-reported confirmed offset stalls during the interruption and resumes advancing on reconnect, with **no** false `success`/`error` event.
- **Connection-reset recovery (8.11):** when M3's `reset_peer` toxic resets the shared browser↔backend connection, assert the SSE stream auto-reconnects and re-syncs (snapshot) once the toxic is removed, alongside M2's resume.
- **M3 corroboration (8.10):** extend the existing browser↔backend bandwidth-throttle scenario (7.4) to assert via the SSE channel that confirmed progress keeps advancing and status stays `uploading` (not `abandoned`) under the slow connection.
...
- [ ] Dropping the SSE request mid-upload does not affect the (independent) upload request, and the stream auto-reconnects and re-syncs (8.9)
- [ ] During an M2 dropped-upload + resume, the SSE-reported confirmed offset stalls then resumes, with no false terminal event (8.11)
- [ ] A `reset_peer` toxic that kills the shared browser↔backend connection is recovered: the SSE stream auto-reconnects and re-syncs once the toxic clears (8.11)
- [ ] The M3 browser↔backend bandwidth-throttle scenario (7.4) is verified via the SSE channel: confirmed progress keeps advancing and status stays `uploading`, not `abandoned` (8.10)
- [ ] `uploads` table has a `bytes_received` column, updated on `POST_RECEIVE` (throttled) and set to `bytes_total` on `POST_FINISH`; the snapshot-on-connect reads it directly (8.12)
- [ ] M2's status model is amended to include `abandoned` as a user-visible state, surfaced via this SSE channel (8.13)
- [ ] All M1–M3 acceptance criteria continue to pass

---

## 9. Milestone 5 — Batch Upload (Refined)

**Scope:** everything in M2 and M4 (the SSE progress channel — §8 — which this milestone's progress UI builds on from the start), plus (M3 — §7 — is a testing-only milestone and adds no application features, so it doesn't change feature dependencies):
...
- Queue list UI: one row per file showing filename, size, per-file progress bar, and status badge (per M4/§8.13's amended model: `idle | uploading | paused | error | success | abandoned | skipped | queued`)
- **Per-file status badges and the aggregate progress bar are driven by the M4 SSE channel** (§8): aggregate progress = sum of `bytesReceived` / sum of `bytesTotal` across all `uploadId`s in the queue, including completed and pending ones. The per-file progress *bar* may still show the optimistic local `onProgress` value (per 8.5) for smoothness, but **status** is always SSE-driven.
...
### Test Requirements
- **Unit/integration tests for queue logic with mocked uploads**: replace the actual `tus-js-client` upload calls with a mock/stub that can simulate success, failure, and slow progress on a per-file basis, without performing real network I/O. For these mocked tests, also mock the SSE map (M4) so queue-orchestration logic can be tested independent of the real channel. Test scenarios:
...
- **E2E test** with a small batch of real small files (e.g., 3× 10MB) to validate the full sequential pipeline end-to-end (not mocked) — including verifying that per-file status badges and the aggregate progress bar are driven by the real M4 SSE channel, not just local `onProgress`
...
- [ ] Aggregate progress bar reflects combined `bytesReceived`/`bytesTotal` from the M4 SSE channel across the whole batch
...
- [ ] All M2 and M4 acceptance criteria continue to pass for individual files within a batch

---

## 10. Milestone 6 — Uploaded Files Visualization & Playback (Refined)

**Scope:** everything in M5, plus:
...
### Frontend Layout
- Two-column layout: left = upload selection/queue (from M5), right = uploaded files list
...
- **Auto-refresh on upload completion (new):** on receiving an SSE `success` event (M4, §8) for any `uploadId`, automatically refetch `GET /files` so the newly-completed file appears in the right-column list without manual refresh — this removes the previous "on refresh" caveat entirely, since the SSE channel now provides the trigger
...
### Test Requirements
- **Test fixtures (§2.3 — different approach from M1–M5):** small (few-second) *real* video files with actual codec streams, **committed** to `tests/fixtures/` — at least one browser-compatible (H.264/AAC MP4) and one non-compatible (e.g., MPEG-2 or ProRes) sample. These cannot be generated on the fly like the M1–M5 size-test files, since `ffprobe` needs genuine codec data.
...
### Acceptance Criteria
- [ ] Page renders two independently-scrollable columns: left = upload/queue (M5 UI), right = uploaded files list
- [ ] `GET /files` returns metadata (id, filename, size, status, duration, resolution, codec, `playable`) for all completed uploads
- [ ] After a successful upload, the file appears in the right-column list **automatically** (triggered by the M4 SSE `success` event — no manual refresh needed)
...
- [ ] All M4 and M5 acceptance criteria continue to pass

---

## 11. Stretch Goal — Playback/Use During Upload (Scoping Notes)

The idea of using a file "while it's being uploaded" to S3-compatible storage is genuinely advanced and worth scoping carefully **after** M1–M6 are stable:
...
## 12. Future Expansion: TLS/HTTPS via Edge Reverse Proxy (Not Required Now)
...
- Performance impact on the 2GB transfers is negligible on modern hardware (TLS/AES-NI overhead), and since M5's queue is sequential (not parallel), there's no concurrent-handshake bottleneck to worry about.
...
| Live progress channel | **Server-Sent Events** (`GET /progress/stream`, browser-native `EventSource`) — one shared stream per page session, fed by `@tus/server`'s `POST_RECEIVE`/`POST_FINISH` events; snapshot-on-connect from the `uploads` table (§8, M4) |
...
2. **Concurrency for a future milestone (§2.5/2.6):** when parallel batch processing is eventually added, will the per-file pause/resume/skip/retry model from M2/M5 carry over directly, or does it need rethinking for concurrent state management? No action needed now — just flagging for the roadmap.
