...
| 2.10 | **Auth / multi-user — CONFIRMED** | Not mentioned at all. | **Confirmed: simple PoC, no multi-user, no login, no access control anywhere** — for M1–M5. **Superseded for M6** (§10), which introduces basic login and per-user folders. |
...
│   ├── src/
│   ├── config/
│   │   └── users.txt.example  # M6 (§10): username:password per line — copy to users.txt, not committed
│   └── ...
...
## 4. Proposed Milestone 0: Infrastructure & Scaffolding
...
## 7. Milestone 3 — Storage-Layer Network Resilience & Performance Testing
...
## 10. Milestone 6 — Basic User Management

**Scope:** everything in M5, plus pre-configured user accounts, login-gated access, and per-user storage folders. This milestone changes the trust model of the whole application — it's not purely additive.

### Design Decisions & Challenges

| # | Topic | Issue/Challenge | Recommendation |
|---|---|---|---|
| 10.1 | **S3 "folder" semantics** | MinIO/S3 has no real folders — only a flat key namespace with `/` as a display convention. "Check whether a folder exists" can't be answered directly; it must be inferred from object keys. | Check via `ListObjectsV2(prefix='<username>/', maxKeys=1)`. If empty, create a zero-byte placeholder object `<username>/.keep` so the prefix persists even with no uploads yet. If non-empty, do nothing. The SQLite-driven file list (M5) already filters to known uploads, so `.keep` never appears in the UI. |
| 10.2 | **Reconciliation timing** | The spec doesn't say *when* the username→folder check happens. | Run it once for **all configured users** at backend startup (idempotent, cheap, and matches "pre-configured" — the user list is static). Optionally re-check a single user at login time too, as a safety net if the config file was edited without restarting the backend. |
| 10.3 | **Auth mechanism — CONFIRMED** | The spec says "user logs in" but doesn't define the mechanism. Two reasonable PoC options: (a) HTTP Basic Auth — zero session code, browser-native prompt, stateless; or (b) a login form + server-side session cookie. | **Confirmed (b)**: session cookie (`express-session`, in-memory store) gives an explicit "current user" needed for upload destination (10.6), file-list filtering, and ownership checks, and matches "when a user logs in, the frontend will show..." with a real login action. |
| 10.4 | **Retrofitting auth onto M1–M5 endpoints** | This milestone changes the trust model of *every* existing endpoint, not just adding a new one. Upload, list, stream, heartbeat, and abandon all become session-gated and username-aware. | Add session middleware in front of all routes except `/login` (and `/config`, which has no sensitive content). Each protected route reads `req.session.username`. |
| 10.5 | **Storage key / schema changes** | Object keys and the `uploads` table currently carry no concept of ownership. | Add a `username` column to `uploads` (migration). Storage key becomes `<username>/<upload-id>` (upload-id avoids filename collisions). The tus `onUploadCreate` hook reads `req.session.username` to set both. |
| 10.6 | **Upload destination scoping — CONFIRMED** | The spec is explicit about the **read** side ("right column shows... media in the user folder") but silent on the **write** side — where do new uploads from a logged-in user land? | **Confirmed:** uploads from a logged-in user are written under that user's prefix (`<username>/...`). |
| 10.7 | **Cross-user access control** | Without scoping, user A could enumerate/stream user B's videos by iterating file IDs. | `GET /files` filters by `req.session.username`. `GET /files/:id/stream` checks the row's `username` matches the session before proxying — `403`/`404` otherwise. |
| 10.8 | **Pre-existing (pre-M6) data** | Files uploaded during M1–M5 testing have no `username` and sit at the bucket root, not under any `<username>/` prefix. | These won't appear in any user's list after M6 — by design, per the one-directional username→folder rule (orphan data is left alone, not reconciled the other way). Acceptable for a PoC; starting M6 testing with a clean MinIO bucket / fresh SQLite DB avoids confusion about "missing" files. |
| 10.9 | **Credentials file format & location** | "Pre-configured in a text file" needs a concrete format and location. | `backend/config/users.txt`, one `username:password` pair per line, plain text per stakeholder decision. Loaded into memory at backend startup — **not** duplicated into SQLite. Mounted as a docker volume so it's editable without rebuilding the image. An `users.txt.example` is committed; the real `users.txt` is not (per §3). |
| 10.10 | **Clear-text passwords** | Acknowledged as an accepted PoC tradeoff. | No action — noted here for the record, consistent with how other deliberate tradeoffs are documented in this spec. |
| 10.11 | **Logout** | Not mentioned in the spec, but testing multiple users implies a way to switch users without restarting the browser. | Add `POST /logout`, destroying the session. Small addition, recommended for testability. |
| 10.12 | **Interaction with §2.10/§2.12** | Earlier decisions stated "no auth, no multi-user, no session/user identification." | M6 **explicitly supersedes §2.10** (that's the point of this milestone) but does **not** reopen §2.12's "no cross-reload upload resume" — the login session cookie serves identity/folder-scoping, a different concern from upload-resume state. |

### Frontend
- Login form (username + password fields; no aesthetic requirements) — gates the entire app. Unauthenticated users see only the login form.
- On successful login, render the M4/M5 two-column UI and fetch `GET /files` (now session-scoped to the logged-in user)
- On login failure, show an inline error message
- Logout control (simple button) — calls `POST /logout`, returns to the login form
- All existing M1–M5 functionality (upload queue, video player, etc.) operates unchanged internally — requests now simply carry a session cookie

### Backend
- `POST /login`: validates username/password against in-memory credentials loaded from `backend/config/users.txt`; on success, establishes a session (`express-session`)
- `POST /logout`: destroys the session
- Session middleware applied to all routes except `/login` and `/config`
- **Startup reconciliation** (10.1/10.2): for each user in `users.txt`, check/create the `<username>/.keep` placeholder if the prefix has no objects; leave existing prefixes (matching or not matching a configured username) untouched otherwise
- `uploads` table: add `username` column (migration)
- tus `onUploadCreate`: set storage key to `<username>/<upload-id>` and the `uploads` row's `username` from `req.session.username`
- `GET /files`: filter by `username = req.session.username`
- `GET /files/:id/stream`: verify `username` ownership before proxying; `403`/`404` on mismatch
- `POST /uploads/:id/heartbeat` and `/abandon` (from M2): verify ownership for consistency

### Test Requirements
- Login: valid credentials succeed; wrong password rejected; unknown username rejected
- Startup reconciliation:
  - Configured user with no existing prefix → `.keep` placeholder created
  - Configured user with existing objects under their prefix → no changes, no duplicate placeholder
  - Orphan prefix (objects under a prefix not matching any configured username) → left untouched, doesn't appear in any user's `GET /files`
- Upload-while-authenticated: resulting object lands under `<username>/`, and the `uploads` row has the correct `username`
- `GET /files` isolation: with two users (A and B) each having uploaded files, A's list contains only A's files and vice versa
- `GET /files/:id/stream` for another user's file returns `403`/`404`
- Unauthenticated requests to protected endpoints return `401`
- Logout invalidates the session — subsequent protected requests return `401` until logging in again
- E2E: login as A, upload a file, see it in the right column; logout; login as B, confirm A's file is not visible; upload a different file as B and confirm isolation holds both ways

### Acceptance Criteria
- [ ] Unauthenticated users see only a login form; all app functionality is gated behind login
- [ ] Valid credentials (from `backend/config/users.txt`) log the user in; invalid credentials show an error and do not log in
- [ ] On backend startup, every configured user has a corresponding `<username>/` prefix in MinIO (via `.keep` placeholder or pre-existing objects)
- [ ] Prefixes not matching any configured username are left untouched and do not appear in any user's file list
- [ ] Files uploaded by a logged-in user are stored under that user's prefix and tagged with their username in `uploads`
- [ ] `GET /files` (right column) returns only the logged-in user's files
- [ ] A user cannot stream another user's file (`403`/`404` on cross-user access)
- [ ] All M1–M5 functionality (upload, resume, batch, playback) continues to work for an authenticated user, scoped to their own folder
- [ ] Logout ends the session; subsequent protected requests return `401` until re-login
- [ ] All M5 acceptance criteria continue to pass, re-validated under the authenticated/scoped flow
...
| Authentication (M6) | `express-session` (in-memory store) for login sessions; credentials loaded at startup from `backend/config/users.txt` (plain `username:password`, §10.9) |
