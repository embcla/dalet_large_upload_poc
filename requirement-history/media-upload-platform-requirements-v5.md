...
| 2.10 | **Auth / multi-user — CONFIRMED** | Not mentioned at all. | **Confirmed: simple PoC, no multi-user, no login, no access control anywhere** — for M1–M4. **Superseded for M5** (§9), which introduces basic login and per-user folders. |
...
│   ├── src/
│   ├── config/
│   │   └── users.txt.example  # M5 (§9): username:password per line — copy to users.txt, not committed
│   └── ...
...
## 9. Milestone 5 — Basic User Management (New)

**Scope:** everything in M4, plus pre-configured user accounts, login-gated access, and per-user storage folders. This milestone changes the trust model of the whole application — it's not purely additive.

### Design Decisions & Challenges

| # | Topic | Issue/Challenge | Recommendation |
|---|---|---|---|
| 9.1 | **S3 "folder" semantics** | MinIO/S3 has no real folders — only a flat key namespace with `/` as a display convention. "Check whether a folder exists" can't be answered directly; it must be inferred from object keys. | Check via `ListObjectsV2(prefix='<username>/', maxKeys=1)`. If empty, create a zero-byte placeholder object `<username>/.keep` so the prefix persists even with no uploads yet. If non-empty, do nothing. The SQLite-driven file list (M4) already filters to known uploads, so `.keep` never appears in the UI. |
| 9.2 | **Reconciliation timing** | The spec doesn't say *when* the username→folder check happens. | Run it once for **all configured users** at backend startup (idempotent, cheap, and matches "pre-configured" — the user list is static). Optionally re-check a single user at login time too, as a safety net if the config file was edited without restarting the backend. |
| 9.3 | **Auth mechanism — CONFIRMED** | The spec says "user logs in" but doesn't define the mechanism. Two reasonable PoC options: (a) HTTP Basic Auth — zero session code, browser-native prompt, stateless; or (b) a login form + server-side session cookie. | **Confirmed (b)**: session cookie (`express-session`, in-memory store) gives an explicit "current user" needed for upload destination (9.6), file-list filtering, and ownership checks, and matches "when a user logs in, the frontend will show..." with a real login action. |
| 9.4 | **Retrofitting auth onto M1–M4 endpoints** | This milestone changes the trust model of *every* existing endpoint, not just adding a new one. Upload, list, stream, heartbeat, and abandon all become session-gated and username-aware. | Add session middleware in front of all routes except `/login` (and `/config`, which has no sensitive content). Each protected route reads `req.session.username`. |
| 9.5 | **Storage key / schema changes** | Object keys and the `uploads` table currently carry no concept of ownership. | Add a `username` column to `uploads` (migration). Storage key becomes `<username>/<upload-id>` (upload-id avoids filename collisions). The tus `onUploadCreate` hook reads `req.session.username` to set both. |
| 9.6 | **Upload destination scoping — CONFIRMED** | The spec is explicit about the **read** side ("right column shows... media in the user folder") but silent on the **write** side — where do new uploads from a logged-in user land? | **Confirmed:** uploads from a logged-in user are written under that user's prefix (`<username>/...`). |
| 9.7 | **Cross-user access control** | Without scoping, user A could enumerate/stream user B's videos by iterating file IDs. | `GET /files` filters by `req.session.username`. `GET /files/:id/stream` checks the row's `username` matches the session before proxying — `403`/`404` otherwise. |
| 9.8 | **Pre-existing (pre-M5) data** | Files uploaded during M1–M4 testing have no `username` and sit at the bucket root, not under any `<username>/` prefix. | These won't appear in any user's list after M5 — by design, per the one-directional username→folder rule (orphan data is left alone, not reconciled the other way). Acceptable for a PoC; starting M5 testing with a clean MinIO bucket / fresh SQLite DB avoids confusion about "missing" files. |
| 9.9 | **Credentials file format & location** | "Pre-configured in a text file" needs a concrete format and location. | `backend/config/users.txt`, one `username:password` pair per line, plain text per stakeholder decision. Loaded into memory at backend startup — **not** duplicated into SQLite. Mounted as a docker volume so it's editable without rebuilding the image. An `users.txt.example` is committed; the real `users.txt` is not (per §3). |
| 9.10 | **Clear-text passwords** | Acknowledged as an accepted PoC tradeoff. | No action — noted here for the record, consistent with how other deliberate tradeoffs are documented in this spec. |
| 9.11 | **Logout** | Not mentioned in the spec, but testing multiple users implies a way to switch users without restarting the browser. | Add `POST /logout`, destroying the session. Small addition, recommended for testability. |
| 9.12 | **Interaction with §2.10/§2.12** | Earlier decisions stated "no auth, no multi-user, no session/user identification." | M5 **explicitly supersedes §2.10** (that's the point of this milestone) but does **not** reopen §2.12's "no cross-reload upload resume" — the login session cookie serves identity/folder-scoping, a different concern from upload-resume state. |
...
- On successful login, render the M3/M4 two-column UI and fetch `GET /files` (now session-scoped to the logged-in user)
- On login failure, show an inline error message
- Logout control (simple button) — calls `POST /logout`, returns to the login form
- All existing M1–M4 functionality (upload queue, video player, etc.) operates unchanged internally — requests now simply carry a session cookie
...
- **Startup reconciliation** (9.1/9.2): for each user in `users.txt`, check/create the `<username>/.keep` placeholder if the prefix has no objects; leave existing prefixes (matching or not matching a configured username) untouched otherwise
...
- [ ] All M1–M4 functionality (upload, resume, batch, playback) continues to work for an authenticated user, scoped to their own folder
- [ ] Logout ends the session; subsequent protected requests return `401` until re-login
- [ ] All M4 acceptance criteria continue to pass, re-validated under the authenticated/scoped flow
...
- Performance impact on the 2GB transfers is negligible on modern hardware (TLS/AES-NI overhead), and since M3's queue is sequential (not parallel), there's no concurrent-handshake bottleneck to worry about.
...
| Metadata DB | SQLite — **embedded in-process within the `backend`**, a single file on its volume; not a separate container/service (unlike MinIO, which is its own service) |
...
| Authentication (M5) | `express-session` (in-memory store) for login sessions; credentials loaded at startup from `backend/config/users.txt` (plain `username:password`, §9.9) |
