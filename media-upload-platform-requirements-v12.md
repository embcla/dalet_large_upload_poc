...
| 12.10 | **Hash mismatch handling** | If client and server hashes differ, what happens to the upload? | Set `status = 'error'` with a distinguishing message (e.g., `"integrity_failure"`) and push an SSE terminal event with `hash_verified: false`. The file remains in MinIO (don't auto-delete — the mismatch itself is diagnostic data) but is not listed in `GET /files` as playable. The frontend shows a distinct "integrity check failed" badge. |
...
- **Batch manifest:**
  - Extend `onUploadCreate` (tus hook): if `batchKey` is present in upload metadata, write `batch_key`, `last_modified`, and `batch_position` to the `uploads` row.
...
- **Post-completion integrity:**
  ...
  - New endpoint `POST /uploads/:id/client-hash`: receives `{ hash: string }` from the frontend after client-side computation completes. Stores in `client_file_hash`. If `server_file_hash` is already present, compare immediately: set `hash_verified = (client === server)`, update `status = 'error'` on mismatch, push SSE `integrity` event `{ uploadId, hash_verified, status }`.
...
- **Ping/pong:**
  ...
  - On receiving a `ping` SSE event: call `POST /batches/:batchKey/pong` (where `batchKey` is the active batch's key, or a per-upload sentinel for single-file flow). Fire-and-forget; no UI change.
...
- **Status model:** add `corrupt` (or `integrity_failure`) as a distinct display state alongside the existing `idle | uploading | paused | error | success | abandoned`.
...
- **Integrity — mismatch:** stub the server hash to return a wrong value; assert `hash_verified = false`, `status = 'error'`, and the SSE `integrity` event carries `hash_verified: false`; assert the frontend shows the "integrity check failed" badge.
...
- [ ] On hash mismatch: `hash_verified = false`, `status = 'error'`, SSE `integrity` event carries `hash_verified: false`, frontend shows "integrity check failed" badge
...
| 13.11 | **X button visibility** | Which states show the X? | Show X for: `uploading`, `paused`, `queued`, `error`, `abandoned`, `cancelled` (to allow removing from the visible list). Hide X for: `success` (the file is done and should remain visible). |
...
## 14. Stretch Goal — Playback/Use During Upload (Scoping Notes)

The idea of using a file "while it's being uploaded" to S3-compatible storage is genuinely advanced and worth scoping carefully **after** M1–M9 are stable:
...
## 15. Future Expansion: TLS/HTTPS via Edge Reverse Proxy (Not Required Now)
...
## 16. Recommended Tech Stack Summary
