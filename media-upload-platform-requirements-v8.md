...
**Scope:** no new application features. This milestone adds the Toxiproxy-based test infrastructure and a battery of scenario tests that exercise M1 (single upload) and M2 (resume/pause/heartbeat) under degraded **browser↔backend** network conditions — particularly speed/throughput, since that's the connection a real end-user's network actually affects. M4 (Batch) and later milestones continue to build on M2's feature set unaffected (§2.15).
